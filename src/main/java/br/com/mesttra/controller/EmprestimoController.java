...
import br.com.mesttra.service.EmprestimoService;
...
public class EmprestimoController {
    ...
    // quando alguém fizer um GET nessa URL /emprestimos/, execute este método
    @GetMapping("/")
    ...
    // quando alguém fizer um GET nessa URL seguida de um número (ex:
    // /emprestimos/id/1),
    // execute este método
    @GetMapping("/id/{id}")
    public ResponseEntity<EmprestimoModel> buscar(@PathVariable("id") Integer id) { // @PathVariable("id") é usado para
        // capturar um valor que vem na URL (no
        // caminho da rota) e colocar esse valor
        // dentro de uma variável do método.

        EmprestimoModel emprestimo = service.buscarPorId(id);

        if (emprestimo == null) {
            // Retorna status 404 Not Found se o emprestimo não for encontrado
            return ResponseEntity.notFound().build();
        }

        // Retorna o emprestimo encontrado com status 200 OK
    ...
    // /emprestimos/nome/joão), execute este método
    // Exemplo: GET /emprestimos/nome/joão retornará todos emprestimos cujo nome
    // contenha
    ...
    public ResponseEntity<List<EmprestimoModel>> buscarPorNome(@PathVariable("nome") String nome) {
        List<EmprestimoModel> emprestimos = service.buscarPorNome(nome);

        if (emprestimos.isEmpty()) {
            // Retorna status 404 Not Found se não houver emprestimos
            return ResponseEntity.notFound().build();
        }

        // Retorna status 200 OK com a lista de emprestimos
        return ResponseEntity.ok(emprestimos);
    }

    // CRIAR
    // Anotação para mapear requisições HTTP POST (/emprestimos/) para o método
    // criar(),
    // ou seja
    // quando alguém fizer um POST nessa URL, execute este método
    @PostMapping("/")
    public ResponseEntity<?> criar(@RequestBody EmprestimoModel emprestimo) {
        try {
            EmprestimoModel salvo = service.salvar(emprestimo);
            return ResponseEntity.ok(salvo);
        } catch (RuntimeException e) {
            // retorna apenas a mensagem, sem stacktrace
    ...
    // quando alguém fizer um PUT nessa URL seguida de um número (ex:
    // /emprestimos/id/1),
    // execute este método
    @PutMapping("/id/{id}")
    public ResponseEntity<EmprestimoModel> atualizar(@PathVariable("id") Integer id,
            @RequestBody EmprestimoModel emprestimo) {

        EmprestimoModel atualizado = service.atualizar(id, emprestimo);
        ...
        // Retorna o emprestimo atualizado com status 200 OK
    ...
    // /emprestimos/id/1), execute este método
    @DeleteMapping("/id/{id}")
