...
import br.com.mesttra.service.CategoriaService;
...
public class CategoriaController {
    ...
    // execute este método
    @GetMapping("/id/{id}")
    public ResponseEntity<CategoriaModel> buscar(@PathVariable("id") Integer id) { // @PathVariable("id") é usado para
        ...
        if (categoria == null) {
            // Retorna status 404 Not Found se a categoria não for encontrada
            return ResponseEntity.notFound().build();
        }

        // Retorna a categoria encontrada com status 200 OK
    ...
    public ResponseEntity<List<CategoriaModel>> buscarPorNome(@PathVariable("nome") String nome) {
        List<CategoriaModel> categorias = service.buscarPorNome(nome);

        if (categorias.isEmpty()) {
            // Retorna status 404 Not Found se não houver categorias
            return ResponseEntity.notFound().build();
        }

        // Retorna status 200 OK com a lista de categorias
    ...
    // execute este método
    @PutMapping("/id/{id}")
    public ResponseEntity<CategoriaModel> atualizar(@PathVariable("id") Integer id, @RequestBody CategoriaModel categoria) {

        CategoriaModel atualizado = service.atualizar(id, categoria);
