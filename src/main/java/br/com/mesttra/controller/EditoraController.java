...
import br.com.mesttra.service.EditoraService;
...
public class EditoraController {
    ...
    // execute este método
    @GetMapping("/id/{id}")
    public ResponseEntity<EditoraModel> buscar(@PathVariable("id") Integer id) { // @PathVariable("id") é usado para
                                                                                 // capturar um valor que vem na URL (no
                                                                               // caminho da rota) e colocar esse valor
                                                                               // dentro de uma variável do método.

        EditoraModel editora = service.buscarPorId(id);

        if (editora == null) {
            // Retorna status 404 Not Found se a editora não for encontrada
    ...
    public ResponseEntity<List<EditoraModel>> buscarPorNome(@PathVariable("nome") String nome) {
        List<EditoraModel> editoras = service.buscarPorNome(nome);

        if (editoras.isEmpty()) {
            // Retorna status 404 Not Found se não houver editoras
            return ResponseEntity.notFound().build();
        }

        // Retorna status 200 OK com a lista de editoras
    ...
    @PostMapping("/")
    public EditoraModel criar(@RequestBody EditoraModel editora) {
    ...
    // execute este método
    @PutMapping("/id/{id}")
    public ResponseEntity<EditoraModel> atualizar(@PathVariable("id") Integer id, @RequestBody EditoraModel editora) {

        EditoraModel atualizado = service.atualizar(id, editora);
        ...
        // Retorna a editora atualizada com status 200 OK
